...
export default function LoginScreen({ navigation }: LoginProps) {
  ...
  const [request, response, promptAsync] = Google.useAuthRequest({
    ...
    // 2. iOS/Android Client IDs (We use the Web ID here for the Proxy flow)
    ...
    // 3. 🔥 THE FIX: Use the smart generator again.
    // Now that we whitelisted 'localhost' in Google Cloud, this will work correctly.
    // It creates the link AND tells Expo where your phone is.
  ...
  useEffect(() => {
    if (response?.type === 'success') {
      ...
      Alert.alert("Google Error", "Auth failed. Check console for details.");
    }
  }, [response]);
